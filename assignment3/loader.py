...
class DataLoader(ABC):
    ...
    def build_train_test_split(self, test_size=0.4):
...
class AbaloneData(DataLoader):
    def __init__(self, path='data/abalone_data.csv', verbose=False, seed=1):
        super().__init__(path, verbose, seed)

    def _load_data(self):
        data = pd.read_csv(self._path, sep=",")
        data = data[data['rings'] > 3]
        data = data[data['rings'] < 21]
        mapping = {'M': 1, 'I': 2, 'F': 3}
...
if __name__ == '__main__':
    cd_data = AbaloneData(verbose=True)
